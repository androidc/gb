...
class StaticQA {
    ...
    init() {
        ...
        self.questions = []
        // если есть в memento, то берем из memento иначе используем начальные вопросы
        if Game.shared.questions.questions.count > 0 {
            self.questions = Game.shared.questions.questions
            self.answers = Game.shared.questions.answers
        }
        else {
